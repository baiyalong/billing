...
public class DbUtil {
	...
	//业务管理平台的数据库连接
	public static void initBuisness()
	{
		try {
			...
			config.setMinConnectionsPerPartition(Integer
					.parseInt(bean.getMinConnectionsPerPartition()));
			config.setMaxConnectionsPerPartition(Integer
					.parseInt(bean.getMaxConnectionsPerPartition()));
			config.setPartitionCount(Integer.parseInt(bean.getPartitionCount()));
			config.setAcquireIncrement(Integer.parseInt(bean.getAcquireIncrement()));
			config.setAcquireRetryAttempts(Integer.parseInt(bean.getAcquireRetryDelay()));
			config.setCloseConnectionWatch(Boolean.parseBoolean(bean.getCloseConnectionWatch()));
	...
	//计量的数据库连接
	public static void initEstimate()
	{
		try {
	...
	public static DatabaseConfigBean BusinessConfigBean()
	{
		...
		bean.setMinConnectionsPerPartition(props.getProperty("minConnectionsPerPartition"));
		bean.setMaxConnectionsPerPartition(props.getProperty("maxConnectionsPerPartition"));
	...
	public static DatabaseConfigBean CalculateConfigBean()
	{
		...
		bean.setMinConnectionsPerPartition(props.getProperty("minConnectionsPerPartition1"));
		bean.setMaxConnectionsPerPartition(props.getProperty("maxConnectionsPerPartition1"));
	...
	public static DatabaseConfigBean EstimateConfigBean()
	{
		...
		bean.setMinConnectionsPerPartition(props.getProperty("minConnectionsPerPartition2"));
		bean.setMaxConnectionsPerPartition(props.getProperty("maxConnectionsPerPartition2"));
