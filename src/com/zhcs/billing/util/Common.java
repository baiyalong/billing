...
public class Common {
	...
	static Client client = dcf.createClient("http://172.17.0.15:9001/BillingWebService.asmx?wsdl"); ;
	/**
	 * 是否正确的日期
	 * */
	...
	/**
	 * num 为正数：返回 num 天后的时间
	 * num 为负数：返回 num 天前的时间
	 * reqtime格式（yyyy-MM-dd HH:mm:ss）
	...
	/**
	 * num 为正数：返回 num 月后的时间
	 * num 为负数：返回 num 月前的时间
	 * reqtime格式（yyyyMM）
	 * @param String reqtime
	 * @param int num
	...
	public static String createBillingID(){
		...
		int timeLen = String.valueOf(timeLong).length() > timeNum ? timeNum : String.valueOf(timeLong).length();
		String time = String.valueOf(timeLong).substring(0,timeLen);
		String ranStr=genRandomNum(15);
		String billingID = billingIDStart+time+ranStr;
		log.info("create a billing id:"+billingID+ " length:"+billingID.length());
		return billingID;
	}
	...
	public static String createAccountID(String orderid){
		...
		int timeLen = String.valueOf(timeLong).length() > timeNum ? timeNum : String.valueOf(timeLong).length();
		String time = String.valueOf(timeLong).substring(0,timeLen);
		String billingID = acountIDStart+time+orderid;
		log.info("create a billing id:"+billingID+ " length:"+billingID.length());
		return billingID;
	}
	 ...
	 * @param value  节点值
	 * */
	public static List<String> List(List<Element> rootNode,List<Element> nodes,String property,String value)
	{
	...
	public static String insertBillTBillCycleSeq(){
		try {
			...
			params.add(1);
			int flag=bdao.doSaveOrUpdate("insert into T_BILL_CYCLE_SEQ values(?,?,?)", params);
			params=null;
			if(flag==0){
				log.info("添加或更新失败 ！");
			}else{
	...
	public static String changeJSON(List<String> paramsList,List<String> valuesList){
	...
	public static String GetBillCycle()
	{
		...
		List<HashMap<String, Object>> list =new BillingBaseDao().doSelect(sql, params);
	 ...
	 * @throws Exception
	 */
	public static Object[] accountPayInerface(String wsdl,String method,String json) throws Exception{
