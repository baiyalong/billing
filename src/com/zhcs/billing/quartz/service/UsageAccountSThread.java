...
public class UsageAccountSThread extends Thread {
	...
	private static LoggerUtil logUtil = LoggerUtil
			.getLogger(UsageAccountSThread.class);
	...
	public synchronized void run() {
		try {
			if (threadOneNum > 0 && tTNum > 0) {
				...
				log.info("UsageAccountSThread Class run Method:线程名称："
						+ threadName + " 开始！");
				logUtil.info("UsageAccountSThread Class run Method:线程名称："
						+ threadName + " 开始！");
				...
				for (int n = 0; n < forNum; n++) {
					...
					log.info("线程名称：" + threadName + " 循环第 " + n + 1 + "次处理 "
							+ threadOneNum + "条数据！！");
					logUtil.info("线程名称：" + threadName + " 循环第 " + n + 1
							+ "次处理 " + threadOneNum + "条数据！！");

					/*** 修改STATE 为 threadName ，表示此线程正在扫描当中 ***/
					int stateNum1 = Scanning(threadName);
					if (stateNum1 == 0) {
						log.error("UsageAccountSThread Class run Method 修改  T_USAGE_ACCOUNT 表 STATE = "
								+ threadName + " 失败 ！");
						logUtil.error("UsageAccountSThread Class run Method 修改  T_USAGE_ACCOUNT 表 STATE = "
								+ threadName + " 失败 ！");
						return; // 修改失败
					} else {
						/*** 查询需要处理的数据 ***/
						...
						usageAccountBean.setSCANNING_WAY("5");// 扫描方式
																// 1:每天扫描一次，5:每5分钟扫描一次
						...
						log.info("UsageAccountSThread Class run Method: 查询需要处理数据为："
								+ list.size() + "条");
						logUtil.info("UsageAccountSThread Class run Method: 查询需要处理数据为："
								+ list.size() + "条");
						/*** 处理数据 ***/
