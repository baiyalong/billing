...
public class UsageAmountVMThread extends Thread {
	...
	private static LoggerUtil logUtil = LoggerUtil.getLogger(UsageAmountVMThread.class);
	private static Logger log = LoggerFactory.getLogger(UsageAmountVMThread.class);
	...
	public synchronized void run(){
		try {
			if (threadOneNum > 0 && tTNum > 0) {
				...
				log.info("UsageAmountVMThread Class run Method:线程名称：" + threadName + " 开始！");
				logUtil.info("UsageAmountVMThread Class run Method:线程名称：" + threadName + " 开始！");
				...
				for (int n = 0; n < forNum; n++) {
					...
					if (stateNum1 == 0) {
						log.error("UsageAmountVMThread Class run Method 修改  T_USAGE_ACCOUNT 表 STATE = " + threadName + " 失败 ！");
						logUtil.error("UsageAmountVMThread Class run Method 修改  T_USAGE_ACCOUNT 表 STATE = " + threadName + " 失败 ！");
						return; // 修改失败
					} else {
						/*** 查询需要处理的数据 ***/
						UsageAccountDao usageAccountDao = new UsageAccountDao();
						UsageAccountBean usageAccountBean = new UsageAccountBean();
						usageAccountBean.setSCANNING_WAY("1");//扫描方式 1:每天扫描一次，5:每5分钟扫描一次
						...
						log.info("UsageAmountVMThread Class run Method: 查询需要处理数据为："+ list.size() + "条");
						logUtil.info("UsageAmountVMThread Class run Method: 查询需要处理数据为："+ list.size() + "条");
						/*** 处理数据 ***/
						if (list.size() > 0 && list != null) {
							...
							//for (int i = 0; i < list.size(); i++) {
								...
								//String amount = BillingMethod.compute(orderInfoBean, "1", list.get(i).getSCANNING_TIME_POINT());
								String amount = BillingMethod.compute(orderInfoBean, "1", "2014-05-28 12:00:00");
								String customerid = getCustomerIdByOrderId(orderInfoBean.getORDER_ID());	//跟据订单编号获得客户编号
								...
								if("true".equals(result)){
									log.info("订单编号为："+orderInfoBean.getORDER_ID()+"调用平台扣费成功！");
									logUtil.info("订单编号为："+orderInfoBean.getORDER_ID()+"调用平台扣费成功！");
								}else {
									log.info("订单编号为："+orderInfoBean.getORDER_ID()+"调用平台扣费失败！");
									logUtil.info("订单编号为："+orderInfoBean.getORDER_ID()+"调用平台扣费失败！");
								}
