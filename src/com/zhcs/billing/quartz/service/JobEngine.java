...
public class JobEngine implements Job {
	...
	public void execute(JobExecutionContext context)
		...
		try {
			 ...
			 */
			JobTaskBean[] bean = QuartzDao
					.getJobTaskBean(VariableConfigManager.execute_select);
			if (bean == null) {
				return;
			}
			for (int i = 0; i < bean.length; i++) {
				...
				// String sql =
				// "UPDATE T_JOB_TASK SET STATE_DATE=now() WHERE TASK_ID=?";
				...
				try {
					...
					inScheduler.deleteJob(bean[i].getTaskCode(),
							bean[i].getTaskType());
					/****************** 把老的任务给停止，然后删除 ******************/
				} catch (Exception e) {
					// 这里如果是在运行过程中，添加新的任务，而不是修改任务，这里会出错的，你说对不对，新加的任务怎么能删除呢？
					log.error(
							"JobEngine Class During operation Insert Job Error !!",
							e);
					logUtil.error(
							"JobEngine Class During operation Insert Job Error !!",
							e);
				...
				/****************** 重新添加任务 ******************/
				job = new JobDetail(bean[i].getTaskCode(),
						bean[i].getTaskType(), Class.forName(bean[i]
								.getTaskImplClass()));
