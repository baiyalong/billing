...
public class MonthsTreatment extends Task implements Job {
	...
	private static LoggerUtil logUtil = LoggerUtil.getLogger(MonthsTreatment.class);
		...
		try{
			...
			if (treatmentNum > 0) {
				...
				for (int i = 0; i < threadNum; i++) {
					...
					MonthsThread mt = new MonthsThread(treatmentNum,tTNum,threadOneNum,serversNo);
					mt.start();
	 ...
	 * @param serversNo
	 */
	private void SetData(int eachMachine, int theMachineStart, int serversNo,List<OrderInfoBean> orders) {
		...
		usageMonthsDao.SetData(usageMonthsBean, eachMachine, theMachineStart, orders);
	}
	...
	private List<OrderInfoBean> HandleDataTotal(){
		...
		List<HashMap<String, Object>> list = queryDao.getMonthsOrderNum(orderInfoBean);
