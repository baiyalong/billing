	...
	public synchronized void run() {
		try {
			if (threadOneNum > 0 && tTNum > 0) {
				...
				log.info("MonthsThread Class run Method:线程名称：" + threadName
						+ " 开始！");
				logUtil.info("MonthsThread Class run Method:线程名称：" + threadName
						+ " 开始！");
				...
				for (int n = 0; n < forNum; n++) {
					...
					if (stateNum1 == 0) {
						log.error("MonthsThread Class run Method 修改  T_USAGE_MONTHS 表 STATE = "
								+ threadName + " 失败 ！");
						logUtil.error("MonthsThread Class run Method 修改  T_USAGE_MONTHS 表 STATE = "
								+ threadName + " 失败 ！");
						return; // 修改失败
					} else {
						/*** 查询需要处理的月租数据 ***/
						...
						List<UsageMonthsBean> list = usageMonthsDao
								.listUsageMonths(usageMonthsBean);
						log.info("MonthsThread Class run Method: 查询需要处理的月租数据为："
								+ list.size() + "条");
						logUtil.info("MonthsThread Class run Method: 查询需要处理的月租数据为："
								+ list.size() + "条");
						/*** 处理月租数据 ***/
						if (list.size() > 0 && list != null) {
							for (int i = 0; i < list.size(); i++) {
								...
								System.gc();
								// 原始保留
								// String account = acc.CommonFeeN(orderid, "2",
								// Common.getDateStr() ,Common.createBillingID()
								// , dateStr, "1");
								String account = acc.CommonFeeN(orderid,
										Common.getDateStr());
								// 進行垃圾回收
								System.gc();
								// 跟据订单编号获得客户编号
								...
								accountPayBean.setAMOUNT(Integer
										.parseInt(account));
