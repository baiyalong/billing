...
public class UsageAccountSTreatment extends Task implements Job {
	...
	private static LoggerUtil logUtil = LoggerUtil.getLogger(UsageAccountSTreatment.class);
	private static Logger log = LoggerFactory.getLogger(UsageAccountSTreatment.class);
	@SuppressWarnings("unchecked")
	@Override
	public void execute(HashMap map) {
		
		Date date = new Date(new Date().getTime()-VariableConfigManager.Delayed_STreatment);
		...
		try{
			...
			if (treatmentNum > 0) {
				...
				for (int i = 0; i < threadNum; i++) {
					...
					UsageAccountSThread ust = new UsageAccountSThread(treatmentNum,tTNum,threadOneNum,serversNo);
					ust.start();
	...
	private List<EstOrderBean> HandleDataTotal(EstOrderBean estOrderBean){
		...
		List<HashMap<String, Object>> list = queryDao.getEstOrderNumFive(estOrderBean);
