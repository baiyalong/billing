...
public class UsageMonthsDao implements IUsageMonthsDao {
	...
	private static LoggerUtil loggerUtil = LoggerUtil
			.getLogger(UsageMonthsDao.class);
	...
	public void SetData(UsageMonthsBean usageMonthsBean, int eachMachine,
		...
		for (int i = theMachineStart; i < (eachMachine + theMachineStart); i++) {
			...
			try {
				...
				if (num != 0) {
					log.info("UsageMonthsDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
							+ usageMonthsBean.getSERVERS_NO_STATE() + "成功！！");
					loggerUtil
							.info("UsageMonthsDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
									+ usageMonthsBean.getSERVERS_NO_STATE()
									+ "成功！！");
					n++;
				}
			} catch (Exception e) {
			...
			if (num == 0) {
				log.info("UsageMonthsDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
						+ usageMonthsBean.getSERVERS_NO_STATE() + "失败！！");
				loggerUtil
						.info("UsageMonthsDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
								+ usageMonthsBean.getSERVERS_NO_STATE()
								+ "失败！！");
		...
		log.info("服务器编号为：" + usageMonthsBean.getSERVERS_NO_STATE() + "的数据共有："
				+ n);
		loggerUtil.info("服务器编号为：" + usageMonthsBean.getSERVERS_NO_STATE()
				+ "的数据共有：" + n);
	}
	...
	public int scanning(UsageMonthsBean usageMonthsBean, int threadOneNum) {
		...
		try {
			...
			if (stateNum != 0) {
				log.info("UsageMonthsDao Class Scanning Method:修改T_USAGE_MONTHS 表STATE 为 "
						+ usageMonthsBean.getSTATE() + "成功！！");
				loggerUtil
						.info("UsageMonthsDao Class Scanning Method:修改T_USAGE_MONTHS 表STATE 为 "
								+ usageMonthsBean.getSTATE() + "成功！！");
			}
		} catch (Exception e) {
	...
	public void updateUsageMonthsState(UsageMonthsBean usageMonthsBean,
		...
		try {
			stateNum = GetBillingBaseDao().doSaveOrUpdate(sql, params);
			log.info(" UsageMonthsDao Class run updateUsageMonthsState Method 修改  T_USAGE_MONTHS 表 STATE = 完成   成功.共修改了数据："
					+ stateNum);
			loggerUtil
					.info(" UsageMonthsDao Class run updateUsageMonthsState Method 修改  T_USAGE_MONTHS 表 STATE = 完成   成功.共修改了数据："
							+ stateNum);
		} catch (Exception e) {
			loggerUtil.error("" + ClassName
					+ " Class run updateUsageMonthsState Method ", e);
		}
		params = null;
		if (stateNum == 0) {
			log.error(" UsageMonthsDao Class run updateUsageMonthsState Method 修改  T_USAGE_MONTHS 表 STATE = 完成   失败！！");
			loggerUtil
					.error(" UsageMonthsDao Class run updateUsageMonthsState Method 修改  T_USAGE_MONTHS 表 STATE = 完成   失败！！");
			return; // 修改失败
		}
	}
	...
	public void deleteDate(UsageMonthsBean usageMonthsBean, int tTNum) {
		try {
			...
			// 所有线程共有处理了多少数据
			// log.info("查询T_USAGE_MONTHS表，线程:"+usageMonthsBean.getSTATE()
			// +" 共有处理了:"+num+" 条状态为:"+usageMonthsBean.getCOMPLETE_STATE()+"数据，应该处理的数为："+tTNum);
			// loggerUtil.info("查询T_USAGE_MONTHS表，线程:"+usageMonthsBean.getSTATE()
			// +" 共有处理了:"+num+" 条状态为:"+usageMonthsBean.getCOMPLETE_STATE()+"数据，应该处理的数为："+tTNum);
			// if(num==tTNum){
			...
			log.info("成功删除线程:" + usageMonthsBean.getSTATE()
					+ " 已经处理完成的数据 T_USAGE_MONTHS表,删除个数为:" + tTNum + "状态为:"
					+ usageMonthsBean.getCOMPLETE_STATE());
			loggerUtil.info("成功删除线程:" + usageMonthsBean.getSTATE()
					+ " 已经处理完成的数据 T_USAGE_MONTHS表,删除个数为:" + tTNum + "状态为:"
					+ usageMonthsBean.getCOMPLETE_STATE());
			// }
			paramsD = null;
	...
	@Override
	public List<UsageMonthsBean> listUsageMonths(UsageMonthsBean usageMonthsBean) {
