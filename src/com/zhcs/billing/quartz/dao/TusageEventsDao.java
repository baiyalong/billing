...
public class TusageEventsDao implements ITusageEventsDao {
	...
	public int insertTUsageEvents(
			TUsageEventsBean tUsageEventsBean, String dateStr, String billingID) {
		int insertId=0;
		String sqlI = "INSERT INTO T_USAGE_EVENTS_"+ dateStr +" " +
		...
		try {
			...
			log.info("详单表T_Usage_Events_"+dateStr+"成功插入: "+insertId+" 条数据!");
		} catch (Exception e) {
			log.info("详单表T_Usage_Events_"+dateStr+"插入数据失败 "+e.getMessage());
			e.printStackTrace();
		}
		paramsI=null;
		return insertId;
	...
	public List<UsageMonthlyBean> listRate(UsageMonthlyBean usageMonthlyBean) {
		...
		List<HashMap<String, Object>> l = GetBillingBaseDao().doSelect(sql, params);
	 ...
	 * @return
	 */
	public List<HashMap<String, Object>> getUsageMonthly(String orderId,String billCycleId){
	...
	private List<UsageMonthlyBean> chageToObject(List<HashMap<String, Object>> l) {
		...
		for(HashMap<String,Object> d:l)
		{
			...
			t.setUNIT_RATE(d.get("UNIT_RATE")!=null?d.get("UNIT_RATE").toString():null);
			t.setCUSTOMER_ID(d.get("CUSTOMER_ID")!=null?d.get("CUSTOMER_ID").toString():null);
			t.setCOMPLETE_STATE(d.get("COMPLETE_STATE")!=null?d.get("COMPLETE_STATE").toString():null);
			list.add(t);
		}
		return list;
	}
