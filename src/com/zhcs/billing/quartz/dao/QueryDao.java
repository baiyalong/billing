...
public class QueryDao implements IQueryDao {
	...
	/*static MeteringBaseDao meteringBaseDao;	// 查询计量库
	static BaseDao baseDao;						// 查询业务库
	static BillingBaseDao billingBaseDao;	*/					// 查询业务库
	 ...
	 * @return
	 */
	public List<HashMap<String, Object>> getEstOrderNumOne(EstOrderBean estOrderBean){
...
//		GetMeteringBaseDao().closed();
	 ...
	 * @return
	 */
	public List<HashMap<String, Object>> getEstOrderNumFive(EstOrderBean estOrderBean){
		List<HashMap<String, Object>> list = new ArrayList<HashMap<String,Object>>();
		String sql = VariableConfigManager.getOrderNumSql;
		List params = new ArrayList();
...
//		GetMeteringBaseDao().closed();
	 ...
	 * @return
	 */
	public List<HashMap<String, Object>> getMonthsOrderNum(OrderInfoBean orderInfoBean) {
		...
		List<HashMap<String, Object>> list = GetBaseDao().doSelectOrder(sql, params);
//		GetBaseDao().closed();
		params=null;
		return list;
	}
	...
	public String getCustomerIdByOrderId(String orderid){
		...
		if (list!=null&&list.size()>0) {
			customerid = list.get(0).get("ACCOUNT_CODE")!=null?list.get(0).get("ACCOUNT_CODE").toString():null;
		}else {
			customerid = null;
		}
		params=null;
	...
	public void insertAccountPay(AccountPayBean accountPayBean){
		...
		try {
			...
			if(num!=0){
				log.info("订单编号："+accountPayBean.getORDER_ID()+" 插入T_ACCOUNTPAY表数据成功！");
				logUtil.info("订单编号："+accountPayBean.getORDER_ID()+" 插入T_ACCOUNTPAY表数据成功！");
			}
		} catch (Exception e) {
		...
		if(num==0){
			log.info("订单编号："+accountPayBean.getORDER_ID()+" 插入T_ACCOUNTPAY表数据失败！");
			logUtil.info("订单编号："+accountPayBean.getORDER_ID()+" 插入T_ACCOUNTPAY表数据失败！");
			return;
		}
		params=null;
	}
