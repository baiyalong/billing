...
public class UsageAccountDao implements IUsageAccountDao {
	...
	private static LoggerUtil loggerUtil = LoggerUtil
			.getLogger(UsageAccountDao.class);
	...
	public void SetData(UsageAccountBean usageAccountBean, int eachMachine,
		...
		for (int i = theMachineStart; i < (eachMachine + theMachineStart); i++) {
			...
			try {
				...
				if (num != 0) {
					log.info("UsageAccountDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
							+ usageAccountBean.getSERVERS_NO_STATE() + "成功！！");
					loggerUtil
							.info("UsageAccountDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
									+ usageAccountBean.getSERVERS_NO_STATE()
									+ "成功！！");
					n++;
				}
			} catch (Exception e) {
			...
			if (num == 0) {
				log.info("UsageAccountDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
						+ usageAccountBean.getSERVERS_NO_STATE() + "失败！！");
				loggerUtil
						.info("UsageAccountDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"
								+ usageAccountBean.getSERVERS_NO_STATE()
								+ "失败！！");
		...
		log.info("服务器编号为：" + usageAccountBean.getSERVERS_NO_STATE() + "的数据共有："
				+ n);
		loggerUtil.info("服务器编号为：" + usageAccountBean.getSERVERS_NO_STATE()
				+ "的数据共有：" + n);
	}
	...
	public int scanning(UsageAccountBean usageAccountBean, int threadOneNum) {
		// String sql =
		// "UPDATE T_USAGE_ACCOUNT SET STATE = ?,COMPLETE_STATE = ? WHERE ID IN (SELECT T.ID FROM(SELECT ID FROM T_USAGE_ACCOUNT WHERE SCANNING_WAY = ? AND SERVERS_NO_STATE = ? LIMIT 0,?)AS T)";
		...
		try {
			...
			if (stateNum != 0) {
				log.info("UsageAccountDao Class Scanning Method:修改T_USAGE_ACCOUNT 表STATE 为 "
						+ usageAccountBean.getSTATE() + "成功！！");
				loggerUtil
						.info("UsageAccountDao Class Scanning Method:修改T_USAGE_ACCOUNT 表STATE 为 "
								+ usageAccountBean.getSTATE() + "成功！！");
			}
		} catch (Exception e) {
	...
	public void updateUsageAccountState(UsageAccountBean usageAccountBean,
			String ClassName) {
		// String sql =
		// "UPDATE T_USAGE_ACCOUNT SET COMPLETE_STATE = ? WHERE SCANNING_WAY = ? AND STATE = ? AND SERVERS_NO_STATE = ?";
		...
		try {
			stateNum = GetBillingBaseDao().doSaveOrUpdate(sql, params);
			log.info(" UsageAccountDao Class run updateUsageAccountState Method 修改  T_USAGE_ACCOUNT 表 STATE = 完成   成功.共修改了数据："
					+ stateNum);
			loggerUtil
					.info(" UsageAccountDao Class run updateUsageAccountState Method 修改  T_USAGE_ACCOUNT 表 STATE = 完成   成功.共修改了数据："
							+ stateNum);
		} catch (Exception e) {
			log.error("" + ClassName
					+ " Class run updateUsageAccountState Method ", e);
			loggerUtil.error("" + ClassName
					+ " Class run updateUsageAccountState Method ", e);
		}
		params = null;
		if (stateNum == 0) {
			...
			loggerUtil
					.error(" UsageAccountDao Class run updateUsageAccountState Method 修改  T_USAGE_ACCOUNT 表 STATE = 完成   失败！！");
			return; // 修改失败
		}
	}
	...
	public void deleteDate(UsageAccountBean usageAccountBean, int tTNum) {
		try {
			// String sql =
			// "SELECT COUNT(*) NUM FROM T_USAGE_ACCOUNT WHERE SCANNING_WAY = ? AND STATE = ? AND COMPLETE_STATE = ?";
			...
			// GetBillingBaseDao().doSelect(sql, params);
			// int num = Integer.parseInt(list.get(0).get("NUM").toString()); //
			// 所有线程共有处理了多少数据
			// log.info("查询T_USAGE_ACCOUNT表，线程:"+usageAccountBean.getSTATE()
			// +" 共有处理了:"+num+" 条状态为:"+usageAccountBean.getCOMPLETE_STATE()+"数据，应该处理的数为："+tTNum);
			// loggerUtil.info("查询T_USAGE_ACCOUNT表，线程:"+usageAccountBean.getSTATE()
			// +" 共有处理了:"+num+" 条状态为:"+usageAccountBean.getCOMPLETE_STATE()+"数据，应该处理的数为："+tTNum);
			// if(num==tTNum){
			// String sqlD =
			// "DELETE FROM T_USAGE_ACCOUNT WHERE SCANNING_WAY = ? AND STATE = ? AND COMPLETE_STATE = ?";
			...
			log.info("成功删除线程:" + usageAccountBean.getSTATE()
					+ " 已经处理完成的数据 T_USAGE_ACCOUNT表,删除个数为:" + tTNum + "状态为:"
					+ usageAccountBean.getCOMPLETE_STATE());
			loggerUtil.info("成功删除线程:" + usageAccountBean.getSTATE()
					+ " 已经处理完成的数据 T_USAGE_ACCOUNT表,删除个数为:" + tTNum + "状态为:"
					+ usageAccountBean.getCOMPLETE_STATE());
			// }
			paramsD = null;
	...
	public List<UsageAccountBean> listUsageAccount(
		...
		// String sql =
		// "SELECT * FROM T_USAGE_ACCOUNT T WHERE T.SCANNING_WAY = ? AND T.SERVERS_NO_STATE = ? AND T.STATE = ?";
