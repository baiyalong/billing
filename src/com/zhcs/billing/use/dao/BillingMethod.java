...
public class BillingMethod {
	...
	private static LoggerUtil logUtil = LoggerUtil.getLogger(BillingMethod.class);
	// 订单号,扫描方式,时间
	public static synchronized String compute(OrderInfoBean orderInfoBean,String SCANNING_WAY, String timeInterval) {
		if (orderInfoBean.getORDER_ID() == null || "".equals(orderInfoBean.getORDER_ID())|| timeInterval == null || "".equals(timeInterval)) {
			log.error("传入的参数有误");
		...
		try {
			...
			log.error("订单：" + orderInfoBean.getORDER_ID() + "获取产品出现异常"+e.getMessage());
			logUtil.error("订单：" + orderInfoBean.getORDER_ID() + "获取产品出现异常"+e.getMessage());
			return "0";
		}
...
//					ItemBean.setUSAGE_AMOUNT(subscription.getSUBSCRIBE_AMOUNT());//申购量
		...
		for (ProductItemBean itemBean : itemBeans) {
			...
			for (TScanningAddTotalBean bean : addTotalBeans) {
				if (addTotalBeans == null || addTotalBeans.size() == 0) {
					log.info("订单：" + orderInfoBean.getORDER_ID() + "没有查询到产品维度累积量");
					logUtil.info("订单：" + orderInfoBean.getORDER_ID() + "没有查询到产品维度累积量");
				}else{
					try {
						if ("1".equals(bean.getSCANNING_WAY())) {
							// 相同纬度累取最后一次同一纬度累积量 产品纬度编码
							if (itemBean.getITEM_CODE().equals(bean.getWD_NO())) {
								map.put(bean.getRESOURCE_ID(), bean.getWD_ADD_TOTAL());
								map2.put(bean.getRESOURCE_ID(), bean.getCURRENT_ADD_TOTAL());
							}
						} else if ("5".equals(bean.getSCANNING_WAY())) {// 按5分钟扫描
							// 相同纬度累加为同一纬度累积量 产品纬度编码
							if (itemBean.getITEM_CODE().equals(bean.getWD_NO())) {
								itemBean.setUSAGE_AMOUNT(itemBean.getUSAGE_AMOUNT()+ bean.getWD_ADD_TOTAL());
								itemBean.setCURRENT_ADD_TOTAL(itemBean.getCURRENT_ADD_TOTAL()+bean.getCURRENT_ADD_TOTAL());//同纬度的当次量
							}
						}
					} catch (Exception e) {
						log.error("产品维度："+itemBean.getITEM_NAME()+"设置累积量出现异常" + e.getMessage());
						logUtil.error("产品维度："+itemBean.getITEM_NAME()+"设置累积量出现异常" + e.getMessage());
					}
				}
			}
			...
			if (itemBean.getPD_ID() != null) {
				if (itemBean.getCURRENT_ADD_TOTAL()>0) {
					// 使用量-套餐剩余量
					int ua = itemBean.getCURRENT_ADD_TOTAL() - itemBean.getITEM_AMOUNT();
					// 套餐剩余量-使用量
					int cat = itemBean.getITEM_AMOUNT()-itemBean.getCURRENT_ADD_TOTAL();
				...
				if (itemBean.getUSAGE_AMOUNT()>0) {
					// 计算产品纬度标准批价
					itemBean.setMONEY(itemBean.getUSAGE_AMOUNT()* itemBean.getPRICE());// 标准批价
					log.info(itemBean.getITEM_NAME() + "累积量："+ itemBean.getUSAGE_AMOUNT() + ",单价："+ itemBean.getPRICE() + "产生标准批价费用:"+ itemBean.getMONEY()+"厘");
					logUtil.info(itemBean.getITEM_NAME() + "累积量："+ itemBean.getUSAGE_AMOUNT() + ",单价："+ itemBean.getPRICE() + "产生标准批价费用:"+ itemBean.getMONEY()+"厘");
					//计算产品维度二次批价
					double money = model.getAmountCount(itemBean.getPRODUCT_ID(),itemBean.getRESOURCE_ID(),itemBean.getITEM_CODE(), itemBean.getUSAGE_AMOUNT()+"", itemBean.getPRICE());
					itemBean.setTwoMONEY(money);// 二次批价
					log.info(itemBean.getITEM_NAME() + "累积量："+ itemBean.getUSAGE_AMOUNT() + ",产生二次批价：" + money + "厘");
					logUtil.info(itemBean.getITEM_NAME() + "累积量："+ itemBean.getUSAGE_AMOUNT() + ",产生二次批价：" + money + "厘");					
				}
			}
		}
			...
			for (ProductItemBean item : itemBeans) {
				try {
					if (resource.getRESOURCE_ID().equals(item.getRESOURCE_ID())) {
						resource.setMONEY(add(resource.getMONEY(), item.getMONEY()));// 产品纬度标准批价
						resource.setTwoMONEY(add(resource.getTwoMONEY(),item.getTwoMONEY()));// 产品纬度二次批价
						log.info("产品资源" + resource.getRESOURCE_NAME() + "的标准批价为："+ resource.getMONEY() + "厘,二次批价为："+ resource.getTwoMONEY() + "厘++");
						logUtil.info("产品资源" + resource.getRESOURCE_NAME() + "的标准批价为："+ resource.getMONEY() + "厘,二次批价为："+ resource.getTwoMONEY() + "厘++");
					}
				} catch (Exception e) {
					log.error("把产品维度："+item.getITEM_NAME()+"的费用添加到产品资源："+resource+"出现异常："+e.getMessage());
					logUtil.error("把产品维度："+item.getITEM_NAME()+"的费用添加到产品资源："+resource+"出现异常："+e.getMessage());
				}
		...
		for (ProductResourceBean rootResource : rootResources) {
			...
			if (infoBean.getPRODUCT_ID().equals(rootResource.getPRODUCT_ID())) {
				...
				infoBean.setTwoMONEY(infoBean.getTwoMONEY()+ rootResource.getTwoMONEY());// 二次批价
		...
		bean.setSCANNING_WAY(SCANNING_WAY);// 扫描方式
		//计算产品优惠后金额
		double SUMARY = model.getDiscountAmount(infoBean.getTwoMONEY(),infoBean.getPRODUCT_ID());
		...
		log.info("产品："+infoBean.getPRODUCT_ID()+"优惠前的金额为"+infoBean.getTwoMONEY()+"厘，优惠后的金额为："+infoBean.getSUMARY()+"厘。");
		...
		for (TCulOrderDetailBean tCulOrderDetailBean : culOrderDetailBeans) {
			try {
				...
				if ("0".equals(tCulOrderDetailBean.getCTYPE())) {
					bean.setDEDUCT_COST(add(new Double(tCulOrderDetailBean.getDEDUCT_COST()),new Double(tCulOrderDetailBean.getREALITY())).toString());
					break;
				}
			} catch (Exception e) {
				log.error("查找订单:"+bean.getORDER_ID()+"已经扣除费用DEDUCT_COST出现异常："+e.getMessage());
				logUtil.error("查找订单:"+bean.getORDER_ID()+"已经扣除费用DEDUCT_COST出现异常："+e.getMessage());
			}
		}
		// 这是这个月的第一条，没有前一条记录
	...
	public static void computingResources(ProductResourceBean bean) {
		...
		for (ProductResourceBean resourceBean : list) {
			...
			if (resourceBean.getNODE_TYPE() != 8) {
				log.info("产品资源" + resourceBean.getRESOURCE_NAME() + "的标准批价为：" + bean.getMONEY() + "厘,二次批价为：" + bean.getTwoMONEY() + "厘--");
				logUtil.info("产品资源" + resourceBean.getRESOURCE_NAME() + "的标准批价为：" + bean.getMONEY() + "厘,二次批价为：" + bean.getTwoMONEY() + "厘--");
			}
		}
	}
