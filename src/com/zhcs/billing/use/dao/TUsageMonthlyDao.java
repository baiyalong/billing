...
public class TUsageMonthlyDao {
	...
	/**
	 * 往 资费月租表(T_USAGE_MONTHLY) 添加记录   字段名(主键id,外键,客户编号、按天扣或月初扣、月扣金额、订单编号、产品编号、产品类型、扫描状态,状态,服务器编号状态)
	...
	public void insertTUsageMonthly(UsageMonthlyBean usageMonthlyBean){
		...
		try {
			...
			if(num!=0){
				log.info("TUsageMonthlyDao Class insertTUsageMonthly Method： 入库T_USAGE_MONTHLY表成功,服务器所需要处理的数据状态为服务器编号:"+ usageMonthlyBean.getSERVERS_NO_STATE()+"成功！！");
			}
		} catch (Exception e) {
		...
		if(num==0){
			log.info("TUsageMonthlyDao Class insertTUsageMonthly Method： 入库T_USAGE_MONTHLY表失败,服务器所需要处理的数据状态为服务器编号:"+ usageMonthlyBean.getSERVERS_NO_STATE()+"失败！！");
		}
		param=null;
	}
	/**
	...
	public int insertTUsageEvents(TUsageEventsBean usageEventsBean){
		String insql="insert into T_USAGE_EVENTS_"+Common.getDateStr().substring(0,Common.getDateStr().length()-2)+" values(?,?,?,?,?,?,?,?,?,?)";
		List orderParam=new ArrayList();
		orderParam.add(Common.getStrSSS());							//主键id
		orderParam.add(usageEventsBean.getBILLCYCLE_ID());			//外键  关联 T_Bill_Cycle_Seq
		...
		try {
			...
			if(num!=0){
				log.info("入库详单表T_USAGE_EVENTS_"+Common.getDateStr().substring(0,Common.getDateStr().length()-2)+"成功插入一条数据："+num);
			}
		} catch (Exception e) {
			log.error("入库详单表T_USAGE_EVENTS_"+Common.getDateStr().substring(0,Common.getDateStr().length()-2)+"入库失败"+e.getMessage());
			e.printStackTrace();
		}
		orderParam=null;
		return num;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void SetData(UsageMonthlyBean usageMonthlyBean,int eachMachine, int theMachineStart){
		
		String sql = "UPDATE T_USAGE_MONTHLY SET SERVERS_NO_STATE = ? WHERE ID IN " +
		"(SELECT T.ID FROM(SELECT ID FROM T_USAGE_MONTHLY WHERE SCANNING_WAY = ? ORDER BY ID LIMIT "+theMachineStart+","+eachMachine+")AS T)";
		...
		try {
			...
			if(num != 0){
				log.info("TUsageMonthlyDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"+ usageMonthlyBean.getSERVERS_NO_STATE()+"成功！！");
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		if(num==0){
			log.info("TUsageMonthlyDao Class SetData Method：修改服务器所需要处理的数据状态为服务器编号:"+ usageMonthlyBean.getSERVERS_NO_STATE()+"失败！！");
			return;
		}
		params=null;
	}
	...
	public int scanning(UsageMonthlyBean usageMonthlyBean, int threadOneNum) {
		String sql = "UPDATE T_USAGE_MONTHLY SET STATE = ?,COMPLETE_STATE = ? WHERE ID IN (SELECT T.ID FROM(SELECT ID FROM T_USAGE_MONTHLY WHERE SCANNING_WAY = ? AND SERVERS_NO_STATE = ? LIMIT 0,"+threadOneNum+")AS T)";
			List params=new ArrayList();
			...
			try {
				...
				if(stateNum!=0){
					log.info("TUsageMonthlyDao Class scanning Method:修改T_USAGE_MONTHLY 表STATE 为 "+usageMonthlyBean.getSTATE()+"成功！！");
				}
			} catch (Exception e) {
			...
			if(stateNum==0){
				log.info("TUsageMonthlyDao Class scanning Method:修改T_USAGE_MONTHLY 表STATE 为 "+usageMonthlyBean.getSTATE()+"失败！！");
			}
			params=null;
			return stateNum;
		}
		...
		public void updateUsageMonthlyState(UsageMonthlyBean usageMonthlyBean,
			...
			try {
				stateNum = GetBillingBaseDao().doSaveOrUpdate(sql, params);
				log.info(" TUsageMonthlyDao Class run updateUsageMonthlyState Method 修改  T_USAGE_MONTHLY 表 STATE = 完成   成功.共修改了数据："+stateNum);
			} catch (Exception e) {
				log.error(""+ClassName+" Class run updateUsageMonthlyState Method ", e);
			}
			params=null;
			if (stateNum == 0) {
		...
		public void deleteDate(UsageMonthlyBean usageMonthlyBean, int tTNum) {
			try{
				...
				log.info("查询T_USAGE_ACCOUNT表，线程:"+usageMonthlyBean.getSTATE() +" 共有处理了:"+num+" 条状态为:"+usageMonthlyBean.getCOMPLETE_STATE()+"数据，应该处理的数为："+tTNum);
				if(num==tTNum){
					...
					params=null;
					log.info("成功删除线程:"+usageMonthlyBean.getSTATE() +" 已经处理完成的数据 T_USAGE_MONTHLY表,删除个数为:"+num+"状态为:"+usageMonthlyBean.getCOMPLETE_STATE());
				}
			}catch (Exception e) {
	...
	private List<UsageMonthlyBean> changeToObject(
		...
		for(HashMap<String,Object> d:list){
			...
			usageMonthlyBean.setBILLSEQ_ID(d.get("BILLSEQ_ID")!=null?d.get("BILLSEQ_ID").toString():null);
			usageMonthlyBean.setCUSTOMER_ID(d.get("CUSTOMER_ID")!=null?d.get("CUSTOMER_ID").toString():null);
			usageMonthlyBean.setFIXED_OR_MONTHLY(Integer.parseInt(d.get("FIXED_OR_MONTHLY")!=null?d.get("FIXED_OR_MONTHLY").toString():null));
			usageMonthlyBean.setSCANNING_WAY(d.get("SCANNING_WAY")!=null?d.get("SCANNING_WAY").toString():null);
			usageMonthlyBean.setUNIT_RATE(d.get("UNIT_RATE")!=null?d.get("UNIT_RATE").toString():null);
			usageMonthlyBean.setORDER_ID(d.get("ORDER_ID")!=null?d.get("ORDER_ID").toString():null);
			usageMonthlyBean.setPRODUCT_ID(d.get("PRODUCT_ID")!=null?d.get("PRODUCT_ID").toString():null);
			usageMonthlyBean.setPRODUCT_TYPE(d.get("PRODUCT_TYPE")!=null?d.get("PRODUCT_TYPE").toString():null);
			usageMonthlyBean.setCOMPLETE_STATE(d.get("COMPLETE_STATE")!=null?d.get("COMPLETE_STATE").toString():null);
			usageMonthlyBean.setSTATE(d.get("STATE")!=null?d.get("STATE").toString():null);
			usageMonthlyBean.setSERVERS_NO_STATE(Integer.parseInt(d.get("SERVERS_NO_STATE")!=null?d.get("SERVERS_NO_STATE").toString():null));
			usageMonthlyBean.setPRODUCT_CATEGORY(Integer.parseInt(d.get("PRODUCT_CATEGORY")!=null?d.get("PRODUCT_CATEGORY").toString():null));
