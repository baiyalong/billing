...
public class BillingCumulative {
	 ...
	 * 获取json内容
	 * @param ORDER_ID  订单id
	 * @param ScanningWay 扫描方式（暂时无用）
	 * @param date 日期
	 * @return
	 */
	public static void getJson(String ORDER_ID, String ScanningWay,
			String date) {
		...
		for (Object obj : jarray) {
			...
			commonResloveJson(configAbilities, orderCode, date, productNo, "NL_NO");
			// 虚拟机资源
			...
			commonResloveJson(configNetworks, orderCode, date, productNo, "WL_NO");
			//对象存储资源
			configStorages = job.get("configStorages") + "";
			commonResloveJson(configStorages, orderCode, date, productNo, "DXCC_NO");
			//物理机资源
			configPhysicalSafes = job.get("configPhysicalSafes") + "";
			commonResloveJson(configPhysicalSafes, orderCode, date, productNo, "WLJ_NO");
	 ...
	 * @param resourceGroupNo	资源组编号
	 * @author whn
	 */
	public static void commonResloveJson(String resourceGroupName,String orderCode,String date,String productNo,String resourceGroupNo){
	...
	public static List<HashMap<String, Object>> getTableByResAndOrder(
		...
		if (list != null && list.size() > 0) {
			log
					.info("根据 资源编码和订单编号查询 传入的表的数据 " + list.size() + "\nsql = "
							+ sql);
			return list;
		} else {
			return null;
	...
	public static void main(String[] args) {
		/*String json = "{\"code\",\"other\"}";
		String code = json.substring(2, 6);
		System.out.println(code);*/
		/*new BillingCumulative();
		BillingCumulative.getJson("1494140507000118", "1", "2014/05/06 12:00:00");
	*/
	}
}
