...
public class SubscriptionResourceBean {
	...
	public List<SubscriptionResourceBean> changeToObject(
		...
		for (int i = 0; i < list.size(); i++) {
			...
			bean.setSR_ID(list.get(i).get("SR_ID") != null ? list.get(i)
					.get("SR_ID").toString() : null);
			bean.setSUBSCRIBER_ID(list.get(i).get("SUBSCRIBER_ID") != null ? list
					.get(i).get("SUBSCRIBER_ID").toString()
					: null);
			bean.setPRODUCT_ID(list.get(i).get("PRODUCT_ID") != null ? list
					.get(i).get("PRODUCT_ID").toString() : null);
			bean.setRESOURCE_ID(list.get(i).get("RESOURCE_ID") != null ? list
					.get(i).get("RESOURCE_ID").toString() : null);
			bean.setSP_ID(list.get(i).get("SP_ID") != null ? list.get(i)
					.get("SP_ID").toString() : null);
			bean.setC_RESOURCE_ID(list.get(i).get("C_RESOURCE_ID") != null ? list
					.get(i).get("C_RESOURCE_ID").toString()
					: null);
			beans.add(bean);
		}
		return beans;
